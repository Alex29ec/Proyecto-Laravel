                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    @foreach ($lista as $reserva)
                        <div class="bg-dark p-4 rounded shadow text-white">
                            ...
                            @if ($reserva->image)
                                <p>Tatuaje:</p>
                                <img src="{{ asset($reserva->image) }}" alt="Tatuaje" class="h-60 w-40 img-fluid mt-2 rounded">
                                <a href="{{ asset($reserva->image) }}" download class="btn btn-sm btn-outline-light mt-2">
                                    Descargar imagen
                                </a>
                            @endif
                        </div>
